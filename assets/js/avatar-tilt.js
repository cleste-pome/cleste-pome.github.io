 ...
 * 头像 3D 倾斜 + 反光（卡片效果）
 * ---------------------------------------------------------------------------
 * 鼠标在头像上移动 → 头像按指针位置轻微旋转，同时一道高光跟着指针走；
 * 按住鼠标 → 往里"退"一点（translateZ 负值 + 轻微缩小）。
 * 鼠标移开 → 平滑回正、高光淡出。
 *
 * 几个刻意的克制：
 *   · 最大倾角只给 10°，学术主页上太夸张会显得廉价
 *   · 触屏设备（没有 hover）直接跳过
...
(function () {
  ...
  var glare = box.querySelector('.avatar-glare');
  ...
  /* 反光层要和头像完全重合：头像是有宽高比的（不是正方形），
     所以尺寸、位置都按实际渲染值同步，不能写死 175px */
  function syncGlare() {
    if (!glare) return;
    ...
    glare.style.transformOrigin = '50% 50%';
  }
  ...
  // 窗口尺寸变化后头像大小会变，重新对齐反光层
  window.addEventListener('resize', syncGlare);

  // 字体/图片加载完再同步一次，避免首次 hover 时位置偏
