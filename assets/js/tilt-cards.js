/*!
 * 3D 倾斜卡片 + 反光（头像、论文配图共用）
 * ---------------------------------------------------------------------------
 * 鼠标在图上移动 → 图片按指针位置轻微旋转，同时一道高光跟着指针走；
 * 按住鼠标 → 往里"退"一点（translateZ 负值 + 轻微缩小）。
 * 鼠标移开 → 平滑回正、高光淡出。
 *
 * 反光层是运行时用 JS 插进去的，所以 HTML/markdown 里不用写任何东西，
 * 只要元素匹配下面的 SELECTOR 就自动生效。
 *
 * 几个刻意的克制：
 *   · 最大倾角 7°，学术主页上太夸张会显得廉价
 *   · 触屏设备（没有 hover）直接跳过
 *   · 系统开了"减少动态效果"就完全不启用（无障碍）
 *   · 用 requestAnimationFrame 节流，不在 mousemove 里直接写 style
 * ---------------------------------------------------------------------------
 */
(function () {
  'use strict';

  var SELECTOR = '.author__avatar, .paper-box-image';

  // 无障碍：用户要求减少动态效果时不启用
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  // 触屏设备没有 hover，跳过
  if (window.matchMedia && !window.matchMedia('(hover: hover)').matches) return;

  var MAX_TILT = 7;      // 最大倾斜角度
  var PERSPECT = 700;    // 透视距离，越小纵深感越强
  var PRESS_Z  = -26;    // 按住时后退的距离(px)
  var PRESS_S  = 0.965;  // 按住时缩小的比例

  function attach(box) {
    var img = box.querySelector('img');
    if (!img) return;

    // 反光层：插在图片后面，和图片完全重合
    var glare = document.createElement('span');
    glare.className = 'tilt-glare';
    glare.setAttribute('aria-hidden', 'true');
    box.appendChild(glare);

    var rx = 0, ry = 0;          // 当前旋转角
    var gx = 50, gy = 50;        // 高光位置（百分比）
    var pressed = false;
    var raf = null;

    /* 图片有各自的宽高比和圆角（头像是椭圆、论文图是矩形），
       所以尺寸、位置、圆角都按实际渲染值同步，不能写死 */
    function syncGlare() {
      var w = img.offsetWidth, h = img.offsetHeight;
      if (!w || !h) return;
      glare.style.width  = w + 'px';
      glare.style.height = h + 'px';
      glare.style.left   = img.offsetLeft + 'px';
      glare.style.top    = img.offsetTop  + 'px';
      glare.style.borderRadius = window.getComputedStyle(img).borderRadius;
    }

    function render() {
      raf = null;

      var transform =
        'perspective(' + PERSPECT + 'px) ' +
        'rotateX(' + rx.toFixed(2) + 'deg) ' +
        'rotateY(' + ry.toFixed(2) + 'deg) ' +
        'translateZ(' + (pressed ? PRESS_Z : 0) + 'px) ' +
        'scale(' + (pressed ? PRESS_S : 1) + ')';

      img.style.transform = transform;
      // 反光层跟着一起转，否则会像贴纸一样脱开
      glare.style.transform = transform;
      glare.style.setProperty('--gx', gx.toFixed(1) + '%');
      glare.style.setProperty('--gy', gy.toFixed(1) + '%');
    }

    function schedule() {
      if (raf === null) raf = requestAnimationFrame(render);
    }

    function reset() {
      rx = 0; ry = 0; pressed = false;
      box.classList.remove('is-tilting');
      schedule();
    }

    box.addEventListener('mouseenter', function () {
      syncGlare();
      box.classList.add('is-tilting');
    });

    box.addEventListener('mousemove', function (e) {
      var r = box.getBoundingClientRect();
      if (!r.width || !r.height) return;

      // 指针相对中心的位置，范围 -0.5 ~ 0.5
      var px = (e.clientX - r.left) / r.width  - 0.5;
      var py = (e.clientY - r.top)  / r.height - 0.5;

      // 往右移 → 绕 Y 轴正转；往下移 → 绕 X 轴反转（所以取负）
      ry =  px * MAX_TILT * 2;
      rx = -py * MAX_TILT * 2;

      // 高光位置：指针相对容器左上角的百分比
      gx = (px + 0.5) * 100;
      gy = (py + 0.5) * 100;

      schedule();
    });

    box.addEventListener('mouseleave', reset);

    // 按住 → 往后退
    box.addEventListener('mousedown', function () { pressed = true;  schedule(); });
    document.addEventListener('mouseup', function () {
      if (!pressed) return;
      pressed = false;
      schedule();
    });

    // 鼠标移出窗口时也可能收不到 mouseup，兜一下
    window.addEventListener('blur', function () {
      if (pressed) { pressed = false; schedule(); }
    });

    // 窗口尺寸变化后图片大小会变，重新对齐反光层
    window.addEventListener('resize', syncGlare);
    window.addEventListener('load', syncGlare);
  }

  function init() {
    var nodes = document.querySelectorAll(SELECTOR);
    for (var i = 0; i < nodes.length; i++) attach(nodes[i]);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', init);
  } else {
    init();
  }
})();
